...
function generateMockPrediction() {
  ...
  MOCK_CONDITIONS.forEach((condition, i) => {
    if (i === topIdx) {
      ...
      const share = i === MOCK_CONDITIONS.length - 1 ? remaining : remaining * Math.random() * 0.5;
...
function generateMockTranscript(language) {
  const symptoms = {
    en: [
      {
        text: 'I have itching on my arms for two weeks',
        keywords: ['itching', 'arms', 'two weeks'],
      },
      {
        text: 'Red patches appeared on my face with burning',
        keywords: ['red patches', 'face', 'burning'],
      },
      {
        text: 'Dry flaky skin on my legs since last month',
        keywords: ['dry skin', 'flaky', 'legs'],
      },
    ],
    ur: [
      ...
      {
        text: 'ٹانگوں پر خشک جلد پچھلے مہینے سے ہے',
        keywords: ['خشک جلد', 'ٹانگیں', 'پچھلا مہینہ'],
      },
    ],
  };
...
const MOCK_CLINICS = [
  ...
  {
    name: 'Rawalpindi Teaching Hospital (RTH)',
    city: 'Rawalpindi',
    region: 'Punjab',
